...
## Capability Composition Pattern (003-agentbridge-mvp)

Source-of-truth: `specs/003-agentbridge-mvp/plan.md`

### Overview

The unit of transfer is a named **capability** (free-text, e.g., "cascade-memory functionality"). The source-side Claude Code agent composes a bundle by:

1. **Graph walk**: capability name → deterministic filename + frontmatter keyword match against `~/.claude/` skills/hooks/rules/bin → expand via cross-references (skill mentions another skill, hook injects a rule path, rule references a `~/bin/` script). LLM reasoning runs on top of deterministic candidates, not from scratch.
2. **3-tier selection matrix** (presented via Rich TUI on both export and ingest sides):
   - **CORE**: required assets — always included, byte-identical preservation (R1, R11)
   - **COMPANIONS**: opt-out defaults — assets that co-travel with CORE in typical use
   - **CONTEXT**: opt-in extras — supporting reference files, examples
3. **Semi-package bundle** produced: `manifest.json` (ManifestModel) + `BRIEFING.md` ("Dear Receiving Claude") + `bundle/` asset tree + `rollback.tar.gz` + `rollback.sh` + `confirmations.log`.
4. **Risk tagging** per asset at compose time (Green/Yellow/Red):
   - Green: rule files, skill markdown without embedded tool-call params
   - Yellow: skill markdown with tool-call params, MCP server config, settings.json fragments, read-only bin scripts
   - Red: hooks, bin scripts that write state
5. **Conflict policy defaults** per asset type: skill markdown → ask; rule files → merge (additive); settings.json → merge (idempotent, additive); hooks → ask; bin scripts → overwrite if user confirmed Red; mcpServers → merge with path-rewrite (FR-015).

### Destination-side ingestion

`agentbridge-ingest` skill / `ab ingest` CLI:
- Reads `BRIEFING.md` — understands intent and per-asset instructions without follow-up questions
- Re-presents the 3-tier selection matrix (lets user further trim)
- Executes BUILD + INGEST per briefing's conflict policy; prompts on every Yellow/Red
- Runs smoke test (post-install self-interrogation + drift detection)
- Leaves `rollback.tar.gz` + `rollback.sh` in place for all-or-nothing undo

### New code location

All capability-level code lives in `agent_transfer/bridge/` (new subpackage, does not exist yet as of 2026-05-04). Existing utils are delegated to, not rewritten (R4 — see techContext.md).
